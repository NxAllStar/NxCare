...
| OI-15 {#oi-15} | Store bền vững ngoài Redis (nếu cần lưu qua nhiều run)? / durable store beyond Redis? | [08](08-data-model.md) persistence | Team lead | Nếu cần lưu liên tục / if persistence across runs is needed | Open |
...
| OI-19 {#oi-19} | Cờ đã thanh toán được đặt bởi ai/cách nào (nhân viên đánh dấu vs tích hợp billing viện)? App không xử lý tiền. / who/what sets the paid flag (staff-marked vs hospital billing integration)? the app processes no money | [FR-05](05-functional-requirements.md#fr-05), [INT-02](09-integration-interface.md) | Team lead | Trước build cổng cho phép tiến hành / before the proceed gate | Open |
