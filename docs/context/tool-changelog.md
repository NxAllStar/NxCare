...
| 2026-07-18 | TASK-009: added deps `openai>=1` (Apache-2.0), `pydantic-settings>=2` (MIT), `pocketflow` (MIT; PyPI metadata omits it, bundled LICENSE is MIT - The-Pocket/PocketFlow), transitive `python-dotenv` (BSD-3-Clause) - all permissive/allowed row. Real Journey chat via OpenAI-compatible API at `LLM_API_BASE_URL` (model `nx-chat`); PocketFlow is the first concrete use of the ADR-001 framework, isolated in `src/vaic/agents/core/flow.py`; new single config module `src/vaic/config.py` | FR-06 chat moved from a rule base to a real LLM provider; ADR-001 framework wired for the first time | ruff clean; pytest 104 passed; no NOTICE file yet (Apache-2.0 openai) - flagged for owner |
