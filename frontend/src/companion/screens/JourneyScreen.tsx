    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18, animation: 'vaicFadeUp .25s ease' }}>
        <div style={{ background: '#F7F8F9', borderRadius: 20, padding: 18 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#0E7490', textTransform: 'uppercase' }}>Đang diễn ra</div>
        ...
        {s.whyOpen && (
          <div style={{ fontSize: 14, color: '#5A626B', lineHeight: 1.5, background: 'rgba(14,116,144,.06)', borderRadius: 14, padding: 14 }}>X-quang và siêu âm đều ở tầng 1, gần khu chờ hiện tại nên làm trước để chị đỡ di chuyển. Khám lại với bác sĩ để cuối vì cần đủ hai kết quả này.</div>
        )}
          ...
          {steps.map((js) => (
            <button key={js.id} onClick={() => a.openStep(js.id)} style={{ textAlign: 'left', border: 'none', background: 'none', cursor: 'pointer', display: 'flex', gap: 14, padding: '10px 0' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
                ...
                {js.status === 'active' && (
                  <div style={{ width: 26, height: 26, borderRadius: 999, background: '#0E7490', boxShadow: '0 0 0 4px rgba(14,116,144,.15)' }} />
                )}
                {js.status === 'upcoming' && (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, animation: 'vaicFadeUp .25s ease' }}>
      <div style={{ height: 160, borderRadius: 18, background: '#F7F8F9', border: '1px solid #E2E5E8', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <svg width="90" height="90" viewBox="0 0 24 24" fill="none"><path d="M12 21s7-6.6 7-12a7 7 0 10-14 0c0 5.4 7 12 7 12z" stroke="#0E7490" strokeWidth="1.5" /><circle cx="12" cy="9" r="2.6" stroke="#0E7490" strokeWidth="1.5" /></svg>
      </div>
      <div>
