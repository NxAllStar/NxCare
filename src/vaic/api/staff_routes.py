"""Two coexisting HTTP surfaces for staff, per the API design plan (docs/tasks) for the async/sync
split (same split `careplan_routes.py`/`patient_routes.py` document).

The module-level `router` below is FR-22 staff patient search - scope-filtered exactly like every
other list endpoint (the FR-18 permission matrix already gives `Role.COORDINATOR` `Scope.ALL` and
`Role.DOCTOR`/`Role.TECHNICIAN` `Scope.ASSIGNED` over `Patient` - this route adds only the `query`
text filter on top).

`build_staff_router(repo)` is the demo/sync-Repository factory for the front desk / doctor side of
the consult queue: the write side of `intake_routes.py`'s patient-facing reads - a doctor calls the
next waiting patient, records the finished visit's outcome (diagnosis + ordered tests -> tasks) and
closes it, or puts a called-but-absent patient back in the queue. The queue transitions route
through `agents/intake/consult_desk.py`; recording the consult outcome routes through
...
from ..models import InvalidTransition, Patient, ServiceType, Task
...
# --- Sync/demo Repository layer (consult-queue front desk actions) ---------------------------
