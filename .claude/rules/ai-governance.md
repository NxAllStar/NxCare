...
An agent, or an in-product model, may never take these actions unsupervised: Executing a re-plan affecting more than N patients (blast radius > N) requires coordinator approval before execution (FR-09); no agent auto-executes above the threshold. Marking the proceed/paid flag and sending patient notifications are logged and auditable..
...
3. Notify Team lead. If personal data was exposed it is a privacy incident and follows the
