...
|---|---|---|
| Undecided - owner Team lead (KI-03); org to set the allow list | Allowed | Permissive: use, modify, and distribute with attribution. The obligation is a notice, not the source |
| Undecided - owner Team lead (KI-03); org to set the deny list | Denied | Copyleft and network-copyleft. See below |
...
- The project keeps a third-party notices file covering every shipped dependency. Adding a dependency
  updates it in the same PR. The failure mode is a slow one: notices are never wrong on the
  day the dependency lands, only on the day of the audit two years later.

## Ownership of agent-authored code

Undecided - owner Team lead (KI-04); ownership of agent-authored code not yet stated by the org
...
1. **Did the manifest or lockfile change?** If yes, list every added package (direct AND transitive)
   ...
   `cargo-deny check licenses`, whichever the stack has. Any licence in Undecided - owner Team lead (KI-03); org to set the deny list, any
