...
## Design direction (this project)

The single source for VAIC's visual direction is `docs/specs/10-ui-ux-wireframes.md` -> "Visual design
direction". In short: Tailwind CSS + shadcn/ui; a calm clinical palette with semantic status colours
(green done/available, amber waiting, red disruption, blue in-progress); one humanist sans with
tabular numbers; reassuring plain Vietnamese; model-produced content always carries an "AI" label
(NFR-USE-05). The three signature surfaces are the patient chat+timeline, the coordinator heatmap +
approval queue, and the disruption reasoning stream. Design tokens are recorded once in the frontend
code, not pinned in the spec. VI is the default language with an EN toggle (FR-21); labels localise,
codes and enums stay English.
