...
if [ "$branch" = "main" ] || [ "$branch" = "master" ]; then
  echo "BLOCKED: effective branch is '$branch'. Per .claude/rules/git-workflow.md, do not commit/push directly to main. Create a branch: git checkout -b feat/<slug> and commit again." >&2
  exit 2
fi
exit 0
